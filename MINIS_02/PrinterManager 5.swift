...
enum OneShotPrinter {
    ...
    private static let sendTimeout: TimeInterval = 7.0
    ...
    private static func sendImpl(
        ...
        func finish(success: Bool, reason: String) {
            ...
            if parts.count >= 3, let oid = Int(parts[1]) {
                let st = String(parts[2])
                PrintJournal.shared.log(.init(
...
extension PrinterManager {
    ...
    func printCashPointSplit(
        ...
        let gap = 120   // ✅ 80–150ms is enough when parallel (keep 200 if you prefer)
        ...
        // ---- Build jobs + dedupe keys ----
        let kitchenDk = "bone|\(order.id)|kitchen"
        let bakeryDk  = "bone|\(order.id)|bakery"
        let barDk     = "bone|\(order.id)|bar"
        let backDk    = "bone|\(order.id)|kitchenback|toast"

        let kitchenJob: Data? = kitchenLines.isEmpty ? nil :
            makeJob(order: order, lines: kitchenLines, rotated: fam.supportsRotation, family: fam, lineWidth: 42)

        let backJob: Data? = toastLines.isEmpty ? nil :
            makeJob(
                order: order,
                lines: toastLines,
                rotated: kitchenBackRotated,
        ...
        let bakeryJob: Data? = bakeryLines.isEmpty ? nil :
            makeJob(order: order, lines: bakeryLines, rotated: fam.supportsRotation, family: fam, lineWidth: 42)

        let barJob: Data? = barLines.isEmpty ? nil :
            makeJob(order: order, lines: barLines, rotated: fam.supportsRotation, family: fam, lineWidth: 42)
        ...
        async let okKitchen: Bool = {
            guard let job = kitchenJob else { return true }
            return await OneShotPrinter.sendAwait(
        ...
        async let okBack: Bool = {
            guard let job = backJob else { return true }
            await gapMs(gap)
            return await OneShotPrinter.sendAwait(
        ...
        async let okBakery: Bool = {
            guard let job = bakeryJob else { return true }
            await gapMs(gap * 2)
            return await OneShotPrinter.sendAwait(
                host: activeBakeryIP,
                port: port,
        ...
        async let okBar: Bool = {
            guard let job = barJob else { return true }
            await gapMs(gap * 3)
            return await OneShotPrinter.sendAwait(
                host: activeBarIP,
                port: port,
        ...
        let (kOk, backOk, bOk, barOk) = await (okKitchen, okBack, okBakery, okBar)

        var failed: [String] = []
        if !kOk { failed.append("Kitchen") }
        if !backOk { failed.append("KitchenBack") }
        if !bOk { failed.append("Bakery") }
...
enum PrintEventType: String, Codable {
    case enq = "ENQ"
...
final class PrintJournal {
    static let shared = PrintJournal()

    private let q = DispatchQueue(label: "PrintJournal")
    private let maxBytes: Int = 2_500_000        // ~2.5MB
    private let keepSeconds: Double = 60 * 60 * 24 * 3  // 3 days

    private lazy var fileURL: URL = {
        let base = FileManager.default.urls(for: .cachesDirectory, in: .userDomainMask).first!
        return base.appendingPathComponent("print-journal.jsonl")
    }()

    func log(_ e: PrintEvent) {
        q.async {
            self.appendLine(e)
            self.trimIfNeeded()
        }
        ...
        q.sync {
            ...
            for line in text.split(separator: "\n") {
                if let d = String(line).data(using: .utf8),
                   let ev = try? decoder.decode(PrintEvent.self, from: d) {
                    out.append(ev)
                }
            }
            return out
    ...
    func read(orderId: Int) -> [PrintEvent] {
        readAll().filter { $0.orderId == orderId }.sorted { $0.ts < $1.ts }
    }
    ...
    private func appendLine(_ e: PrintEvent) {
        let encoder = JSONEncoder()
        encoder.outputFormatting = [] // one-line
    ...
    private func trimIfNeeded() {
        ...
        // parse timestamps quickly (cheap JSON scan)
        lines = lines.filter { line in
            // super cheap: find `"ts":`
            guard let r = line.range(of: "\"ts\":") else { return true }
            let after = line[r.upperBound...]
            // number ends at comma
